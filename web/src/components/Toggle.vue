...
<template>
    <div @click="$emit('toggle', false); $emit('toggle_off');" v-if="switchStatus" :title="'Disable ' + switchName"
      :class="disabled === true ? 'cursor-not-allowed pointer-events-none opacity-25' : null"
    ...
    <div @click="$emit('toggle', true); $emit('toggle_on');" v-else :title="'Enable ' + switchName"
      :class="disabled === true ? 'cursor-not-allowed pointer-events-none opacity-25' : null"
